...
- toString은 언제든지 쓰일 수 있기 때문에 재정의 해두면 디버깅하기 쉽다
- 그 객체가 가진 주요 정보는 모두 toString으로 반환하는 게 좋다
- 포맷을 정할 꺼라면 구체적으로 정해서 이후에 데이터 형태로 활용하기에 용이하게 하는 것이 좋다
- 그렇다고 꼭 포맷을 정할 필요는 없지만 중요한 것은 toString 형태에 대한 의도는 분명하게 밝히는 것이 좋다
- 의도를 표현이 필요하기에 AutoValue로 자동 생성을 권장하는 것은 아니지만, 없는 것보다는 낫다.
